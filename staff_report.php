...
// Fetch a unique list of all offices for the filter dropdown
...
// Check if an office is selected from the filter
$selected_office = isset($_GET['office']) ? $_GET['office'] : '';

// The main query with GROUP_CONCAT to combine training records
$main_query = "
    SELECT
        u.username,
        s.position,
...
if (!empty($selected_office)) {
    // Add the filter condition for the office
...
<head>
    ...
    <style>
        ...
        .container { max-width: 1400px; margin-top: 50px; }
