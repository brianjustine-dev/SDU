        ...
        if ($stmt) {
            ...
            if ($result->num_rows > 0) {
                ...
                if (password_verify($password, $user['password'])) {
                    // Correctly store username in a session variable
                    $_SESSION['username'] = $user['username'];
                    $_SESSION['role'] = $user['role'];
                    $_SESSION['user_id'] = $user['id'];

                    // In your login.php file, after successful login
                if ($user['role'] === 'admin') {
...
/* General body and font styles */
    ...
    /* Updated responsive design using media queries */
    ...
    @media (max-width: 768px) {
        /* Main breakpoint for stacking columns on tablets and phones */
        .login-container {
    ...
    @media (max-width: 480px) {
        ...
        .role-selection {
            flex-direction: column; /* Stack roles on small screens if desired */
        }
    }
